...
from dict_handler import dict_handler #import change_dict_art, change_dict, dump_dict
from decoding import Abstractor, RLExtractor, DecodeDataset, BeamAbstractor
from decoding import make_html_safe
import pickle as pkl
# public variables
dict_handler_obj = dict_handler()
#article_summ_dict = {}
...
def decode(save_path, model_dir, split, batch_size,
	...
	n_data = len(dataset)
	loader = DataLoader(
		dataset, batch_size=batch_size, shuffle=False, num_workers=4,  #edited here
		collate_fn=coll
	)
	print("SUYASH: size of dataset:",len(loader.dataset))
		...
		for i_debug, raw_article_batch in enumerate(loader):
			...
			for j, n in ext_inds:
				...
				with open(join(save_path, 'output/{}.dec'.format(i)),
					...
					#print('.'.join(decoded_sents))
					#dict_handler_obj.change_dict(i,'\n'.join(decoded_sents))
				i += 1
